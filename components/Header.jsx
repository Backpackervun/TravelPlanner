...
export default function Header({
  ...
  return (
    <header className="page-header sticky top-0 z-30 border-b border-paper-line bg-paper/85 backdrop-blur-md">
      <div className="mx-auto max-w-[1600px] px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between gap-3 py-3.5 sm:py-4">
          ...
          <div className="flex items-center gap-3 min-w-0">
            <img src="/logo.png" alt="Backpackervun" className="h-7 w-auto flex-shrink-0 sm:h-8" />
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            {/* Region pill — hidden on very small screens to save space */}
            ...
            {/* Rate — hidden on mobile (accessible in menu) */}
            {!isIDR && (
              <div className="hidden md:flex items-center gap-1.5 rounded-lg border border-paper-line bg-white px-3 py-2 shadow-soft">
                ...
                {rateSource === "live" && (
                  <span className="rounded-full bg-emerald-100 px-1.5 py-0.5 text-[9px] font-bold text-emerald-700" title={rateTime ? `Updated at ${rateTime}` : "Live"}>
                    LIVE
                  </span>
                )}
                {rateSource === "error" && (
            ...
            <div className="relative">
              <button
                ...
                className={`no-print inline-flex items-center gap-1.5 rounded-lg border px-2.5 py-2 sm:px-3 text-xs font-semibold transition disabled:opacity-60 ${saveBtnClass}`}
              >
      ...
      {/* ── MOBILE DRAWER — fixed, full height, scrollable ───────────────── */}
      {open && (
        <>
          {/* Backdrop */}
          <div
          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-paper-line px-5 py-4 sticky top-0 bg-white z-10">
              <div>
                <p className="text-sm font-semibold text-ink">
                  {firstName ? `Hi, ${firstName} 👋` : "Menu"}
                </p>
                {user?.email && <p className="text-[11px] text-ink-muted truncate max-w-[180px]">{user.email}</p>}
              </div>
              <button
                onClick={close}
                className="grid h-8 w-8 place-items-center rounded-lg text-ink-muted hover:bg-paper-dim hover:text-navy-500"
              >
            ...
            {/* Rate section — visible on mobile in drawer */}
            {!isIDR && (
              <div className="border-b border-paper-line px-5 py-3 bg-paper-dim/40">
                <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-ink-muted mb-1.5">Exchange Rate</p>
                <div className="flex items-center gap-2">
                  <span className="text-xs text-ink-muted">1 {currency.code} =</span>
                  <input
                    type="number" value={rate}
                    onChange={(e) => onRateChange(Number(e.target.value))}
                    className="w-20 rounded-lg border border-paper-line bg-white px-2 py-1 text-right font-mono text-sm font-semibold text-ink outline-none focus:border-accent-300"
                  />
...
function DItem({ icon, label, onClick, danger }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`flex w-full items-center gap-3 rounded-lg px-4 py-3 text-left text-sm transition hover:bg-paper-dim active:scale-[0.98] ${
