...
// ── Custom date input — styled to match design system ────────────────────────
...
function DateInput({ value, onClick, placeholder }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex w-full items-center gap-2 rounded-lg border border-paper-line bg-white px-3 py-2.5 text-sm text-left transition hover:border-navy-200 hover:shadow-soft focus:border-accent-300 focus:outline-none focus:shadow-[0_0_0_3px_rgba(74,144,226,0.18)]"
    >
...
export default function SetupScreen({
  tripInfo, region, onTripInfoChange, onRegionChange, onStart,
}) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 sm:py-14">
      ...
      {/* Trip details card — overflow-visible so popper isn't clipped */}
      <section className="rounded-2xl border border-paper-line bg-white p-6 shadow-soft sm:p-8" style={{ overflow: "visible" }}>
        ...
        <div className="mt-6 space-y-4">
          ...
          {/* Date pickers — grid, overflow-visible so popup isn't clipped */}
          <div className="grid gap-4 sm:grid-cols-2" style={{ overflow: "visible" }}>
            ...
            <div style={{ overflow: "visible" }}>
              ...
              <DatePicker
                ...
                customInput={<DateInput placeholder="Pick start date" />}
              />
            </div>
            ...
            <div style={{ overflow: "visible" }}>
              ...
              <DatePicker
                ...
                popperPlacement="bottom-start"
                popperClassName="datepicker-popper"
                customInput={<DateInput placeholder="Pick end date" />}
              />
            </div>
          </div>
          ...
          <div>
            <label className="block text-[11px] font-semibold uppercase tracking-[0.16em] text-ink-muted mb-1.5">
              Duration
              <span className="ml-1.5 text-[10px] font-medium text-ink-muted/60 normal-case">(auto-calculated)</span>
            </label>
            <div className="rounded-lg border border-paper-line/60 bg-paper-dim px-3 py-2.5 text-sm font-medium text-ink min-h-[42px] flex items-center">
              {tripInfo.duration ? (
                <span>{tripInfo.duration}</span>
              ) : (
                <span className="text-ink-muted/50 font-normal">Select start and end dates above</span>
              )}
            </div>
      ...
      <div className="mt-8 flex flex-col items-center gap-2">
        <button
          ...
          className={`inline-flex items-center gap-2 rounded-xl px-8 py-3 text-sm font-semibold transition ${
            canStart
              ? "bg-navy-500 text-white shadow-[0_2px_12px_rgba(11,60,93,0.32)] hover:bg-navy-600"
