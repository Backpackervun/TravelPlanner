/**
 * lib/firestore.js — v9
 * createUserProfile accepts dreamDestination field.
 * redeemCode uses setDoc+merge (no "No document to update" crash).
 */
import {
...
export async function createUserProfile(uid, { name, email, phone, dreamDestination }) {
...
export async function redeemCode(userId, userEmail, codeStr) {
  ...
  await setDoc(codeRef, { used: true, usedBy: userId, usedAt: serverTimestamp() }, { merge: true });

  // setDoc+merge creates user doc if missing — no crash
  await setDoc(doc(db, "users", userId), {
    uid: userId, email: userEmail ?? "",
    plan: effect.plan, planStatus: "active", expiresAt,
  }, { merge: true });

  return { success: true, plan: effect.plan, expiresAt, codeType: data.type };
}
...
export async function seedInitialCodes() {
  ...
  for (const c of codes) {
    const s = await getDoc(doc(db, "redeemCodes", c.code));
    if (!s.exists()) await setDoc(doc(db, "redeemCodes", c.code), { ...c, createdAt: serverTimestamp() });
  }
}
...
export async function generateCodes({ type, durationDays, quantity }) {
  ...
  for (let i = 0; i < quantity; i++) {
    ...
    await setDoc(doc(db, "redeemCodes", code), {
      ...
      expiresAt: null, usedBy: null, usedAt: null, createdAt: serverTimestamp(),
    });
    created.push(code);
  }
  return created;
...
export async function adminGetStats() {
  const [usersSnap, codesSnap] = await Promise.all([getDocs(collection(db, "users")), getDocs(collection(db, "redeemCodes"))]);
  const now = new Date();
...
export async function adminGetAllCodes() {
  ...
  return snap.docs.map((d) => ({ id: d.id, ...d.data(), createdAt: d.data().createdAt?.toDate?.()?.toISOString() ?? null, usedAt: d.data().usedAt?.toDate?.()?.toISOString() ?? null }));
}
