...
export async function ensureUserDoc(uid, email = null) {
  ...
  if (!snap.exists()) {
    ...
    if (!data.expiresAt && (plan === "FREE" || !data.plan)) {
      ...
      const expiresAt = new Date(baseDate.getTime() + TRIAL_MS).toISOString();
      const isExpired = new Date() > new Date(expiresAt);

      await updateDoc(ref, {
        // ✅ If trial still active → LITE, else → FREE permanently
        plan:       isExpired ? "FREE" : "LITE",
        planStatus: isExpired ? "free"  : "trial",
        expiresAt,
      });
    }
  }
